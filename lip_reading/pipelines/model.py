...
class AVSR(torch.nn.Module):
    def __init__(self, modality, model_path, model_conf, rnnlm=None, rnnlm_conf=None,
        ...
        if modality == "audiovisual":
            from espnet.nets.pytorch_backend.e2e_asr_transformer_av import E2E  # 音+视模型
        else:
            from espnet.nets.pytorch_backend.e2e_asr_transformer import E2E # 单模态（视觉或音频）模型
    ...
    def infer(self, data):
        with torch.no_grad():
            if isinstance(data, tuple):
                enc_feats = self.model.encode(data[0].to(self.device), data[1].to(self.device)) #把“裁好的嘴部帧序列” → 送入模型 → 编码 → 解码 → 输出最终一句完整话！
            else:
                enc_feats = self.model.encode(data.to(self.device)) #将输入嘴图序列 → 经过 Transformer 编码器 → 得到时间步维度的特征向量 enc_feats通常 shape 类似于 [T, D]，表示：
            nbest_hyps = self.beam_search(enc_feats)
            nbest_hyps = [h.asdict() for h in nbest_hyps[: min(len(nbest_hyps), 1)]]
            transcription = add_results_to_json(nbest_hyps, self.token_list)#把 beam search 结果 token 列表 → 转成可读字符串（"HELLO WORLD."）
            transcription = transcription.replace("▁", " ").strip() #	移除结尾标志 <eos>（终止符）
        return transcription.replace("<eos>", "")

# 创建 beam search解码器 用于 “从 logits → 句子”
